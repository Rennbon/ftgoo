...
func (taskCenter) GetFoldersCreateTimeBetween(startTime time.Time, endTime time.Time) ([]*Folder, error) {
	...
	var folders []*Folder
	//所有符合条件的项目
	err := fcol.Find(query).Select(pro).All(&folders)
	if err != nil {
		return nil, err
	}
	return folders, nil
...
folderId:项目id,time:指定早于的时间(time.Time{}时不进入筛选) */
func (taskCenter) GetTasksByFolderIdAndTime(folderId string,time time.Time) ([]*Task, error) {
	...
	var tasks []*Task
	err := tcol.Find(query).Select(pro).All(&tasks)
	if err != nil {
		return nil, err
	}
	return tasks, err
...
func (taskCenter)InsertFolderStatistics(fsts *FolderStatistics)bool,error{
	if fsts==nil{
		return nil,errors.ERR_PARAMETER.Error()
	}
